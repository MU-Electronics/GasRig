...
namespace App { namespace Hardware { namespace HAL { namespace Presenters
{
    ...
    QVariantMap PressureSensorPresenter::proccess(QString method, QVariantMap commands, QStringList package)
    {
        ...
        {
            return extend(commands, confirmInit(commands, package));
        }
        else if (method == "readSerialNumber" && expectedPackage(commands, package, "69", 8))
        {
            return extend(commands, readSerialNumber(commands, package));
        }
        else if (method == "readPressure" && expectedPackage(commands, package, "73", 9))
        {
            return extend(commands, readPressure(commands, package));
        }
        ...
        {
            // Log error
            qCCritical(halAccessPressureSensorPresenter) << "Could not find the correct flow controll presenter method. " << generateError(method, commands, package);
        ...
        else
        {
            ...
            // Log error
            qCCritical(halAccessPressureSensorPresenter) << "Could not find the correct pressure sensor presenter method. " << error;
